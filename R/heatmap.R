...
create_hard_skills_heatmap <- function(skills_data) {
...
save_hard_skills_heatmap <- function(p, path) {
...
create_soft_skills_heatmap <- function(skills_data) {
  ...
  exp_level_descriptions <- c(
    ...
    "2 - Improving",
    "1 - Novice",
    "0 - Have not had opportunity",
    "NA - Attribute not applicable to skill"
  )
  ...
  ggplot2::ggplot(skills_data, ggplot2::aes(x = Skill, y = Attribute, fill = Level)) +
    ggplot2::geom_tile(color = "white",
                       lwd = 0.3,
                       linetype = 1) +
    ggplot2::geom_text(ggplot2::aes(label = Level, color = Level),
...
save_soft_skills_heatmap <- function(p, path) {
